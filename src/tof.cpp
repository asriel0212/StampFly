/*Time-of-Flight (ToF) センサーを操作するためのモジュールであり、ドローンや自律型ロボットなどで距離測定を行う目的に利用されます。ToFセンサーとして VL53LX チップを使用しています。このセンサーはレーザーを用いて正確な距離測定を実現します。


初期化処理: センサーを設定し、測定を開始。

リアルタイム測定: 距離データを取得し、範囲を計算。

テストとデバッグ: センサーの状態をシリアルモニターで確認。



・利点
高精度測定:
VL53LXセンサーを利用することで、微小な距離変化を測定可能。

柔軟性:
複数の測定モード（短距離、中距離、長距離）に対応。
